...
@retry(stop=stop_after_attempt(2), wait=wait_exponential(multiplier=1, max=3))
def openai_chat_complete(
    ...
    **kwargs,
    ...
    try:
        ...
        import sys as _sys
        # Surface the real error body once per unique type/status so we can see
        # what OpenAI is actually returning (401/429/etc.) instead of tenacity
        # silently retrying and eventually raising RetryError.
        body = getattr(e, "body", None)
        status = getattr(e, "status_code", None)
        print(f"[openai_chat_complete] {type(e).__name__} status={status} body={body}",
...
class APICostCalculator:
    ...
    _model_cost_per_1m_tokens = {
        ...
        # gpt-5.4-mini official pricing (released 2026-03-17): $0.75 / $4.50 per 1M.
